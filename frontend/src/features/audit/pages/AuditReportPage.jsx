...
const AuditReportPage = () => {
    ...
    const loadReport = async () => {
        setLoading(true);
        try {
            const data = await invoke('audit_get_error_report', {
                stationId: filterStation ? filterStation : null,
                year: filterYear ? filterYear : null,
                month: filterMonth ? filterMonth : null
            });
            setReportRows(data || []);
        } catch (error) {
                ...
                <div className="space-y-4">
                    ...
                    {loading ? (
                        <div className="text-center py-12">
                            ...
                            <span className="text-slate-500 text-sm">Cargando inconsistencias...</span>
                        </div>
                    ) : reportRows.length === 0 ? (
