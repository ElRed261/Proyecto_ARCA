...
#[tauri::command]
pub fn get_audit_logs(app_handle: AppHandle, skip: Option<i32>, limit: Option<i32>) -> Result<Vec<AuditLog>, String> {
    ...
    let logs = stmt.query_map([l, s], |row| {
        Ok(AuditLog {
            id: row.get(0)?,
            action: row.get(1)?,
...
#[tauri::command]
pub fn get_correction_requests(app_handle: AppHandle, skip: Option<i32>, limit: Option<i32>) -> Result<Vec<CorrectionRequest>, String> {
    let s = skip.unwrap_or(0);
    let l = limit.unwrap_or(100);
    let db_path = get_db_path(&app_handle);
    let conn = Connection::open(&db_path).map_err(|e| e.to_string())?;
    ...
    let reqs = stmt.query_map([l, s], |row| {
...
#[tauri::command]
pub fn create_correction_request(app_handle: AppHandle, request: CorrectionRequestCreate, current_user_id: Option<i32>) -> Result<HashMap<String, String>, String> {
    let db_path = get_db_path(&app_handle);
    let conn = Connection::open(&db_path).map_err(|e| e.to_string())?;
