...
#[tauri::command]
pub fn get_monthly_summaries(app_handle: AppHandle, skip: Option<i32>, limit: Option<i32>) -> Result<Vec<MonthlySummary>, String> {
    ...
    let summaries = stmt.query_map([l, s], |row| {
...
#[tauri::command]
pub fn create_monthly_summary(app_handle: AppHandle, summary: MonthlySummaryCreate) -> Result<HashMap<String, String>, String> {
    let db_path = get_db_path(&app_handle);
    let conn = Connection::open(&db_path).map_err(|e| e.to_string())?;
    ...
    conn.execute(
        ...
        (&summary.station_id, &summary.year, &summary.month, &data_str),
    ).map_err(|e| e.to_string())?;
